...
small changes in momentum won't cause the supremum signal to fire. To smoothen out the data, we use the Gaussian Kernel along with a Convolution Filter on the base spread dataset.

\subsection{Time-series Convolution Filter}

Let's define our time series as the function $f: t \in \mathbb{N} \rightarrow \mathbb{R}$, we then define a kernel function $g$. We can then define a convolution between the time-series
function $f$ and the kenel $g$ to be a new composed function $(f * g)(i)$, where I is the time-parameter of the new convoluted time-series.
\begin{gather*}
    (f * g)(i) = \sum_{j = 1}^{m} g(j) f \left ( i - j \right )
\end{gather*}
