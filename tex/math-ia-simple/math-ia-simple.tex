...
\usepackage{algpseudocode}              % for algorithm syntax highlighting
\usepackage{cite}                       % for citations
\usepackage[nottoc,numbib]{tocbibind}   % to make bibliography appear in table of contents
\usepackage[numbers]{natbib}            % for bibliography urls
\usepackage{hyperref}                   % for bibliography urls
...
\begin{titlepage}
    ...
    \vspace*{-4cm}
    \begin{center}
        \huge \textbf { Ornstein Uhlenbeck Process Statistical Arbitrage on Cointegrated Equity Pairs } \\
        \vspace{0.5cm}
...
\pagenumbering{gobble} % turn off page numbering for toc
\tableofcontents
\newpage
...
In modern financial markets, algorithmic trading relies heavily on mathematical tools (such as stochastic processes) to identify trading opportunities (signals) and
improve performance. Among these stochastic processes, the ornstein-uhlenbeck process is the pillar of mean reversion strategies, which are designed to capitalize
on deviations in asset prices (or in our case, the difference between asset prices) from their long-term equilibrium.
In this mathematical exploration, we examine the use of the Ornstein-Uhlenbeck process in statistical arbitrage, focusing on its application within equity pairs trading.
We identify pairs of equities that are viable for statistical arbitrage by using the Augmented Dickey Fuller Test for time-series cointegration.
Through a detailed exploration of the mathematics behind stochastic processes, stochastic differential equations (SDEs), numerical methods for solving SDEs, and
backtesting techniques, this IA demonstrates the feasability and effectiveness of mathematical models in quantitative finance. The quantitative findings and return
metrics aim to provide insight into the extent of the role of mathematics in modern trading strategies and highlight areas for further refinement and application.

\section{Brownian Motion and the Wiener Process}

Consider a stochastic process, where per each iteration, we add some change $\Delta$ to the previous value,
this $\Delta$ is distributed by a normal distribution with a $\mu$ (average, of course) of zero, which we then sample in independent
stochastic events. The result will be a Wiener Process ~\cite{lalley_brownian_motion}. In discrete time, we can define the Wiener process
...
a possible outcome of running the process, essentially a random path that conforms to the process.
\begin{center}
...
but we will get to that later. We now add the drift term (which we derived in a previous subchapter) $\theta(\mu - X_{t})$ (we derived it as $-\theta(X_{t} - \mu)$, but these are functionally equivalent), we can say that this drift happens
...
is just basic differential equations. We end up with: ~\cite{mediatum_stochastic_calculus}
\begin{gather*}
...
in quantitative finance and econometrics.But what is cointegration? Cointegration represents the level to which a linear combination
...
DF test. First, we transform our original time series expression in as a differencing equation. Note that we also rewrite the $y_{t} - y_{t - 1}$ as $\Delta y_{t}$. ~\cite{hurvich_unit_root}
\begin{gather*}
...
We now perform the two-step engle granger test on 78 mature stocks from the selected from NADSAQ-100 ~\cite{nasdaq} index from Janurary 1st 2014 to January 1st 2024. We pair every stock with every other
...
\end{center}
~\cite{seabold_pytts} Only 100 pairs are displayed as the complete 78x78 heatmap is too large to present here.
...
you can use a much more rigorous technique that yields better results, called: Maximum Likelyhood Estimation (MLE), we do not have the capacity to explain MLE in this IA due to it requiring
multivariable calculus (A formal definition of multivariable OLS regressions also requires matrix calculus, it's very simple, we just don't have the capacity for it) and a bit more
rigorous understandng of probability theory, additionally, OLS parameter estimation is frequently used in Quantitative Finance, despite it's obvious limitations. The first step is
obtaining a formal solution to the OU stochastic differential equation (SDE).
...
integrating factor. ~\cite{uregina_ou_solution}
\begin{gather*}
    dX_{t} = \theta \mu dt - \theta X_{t} dt + \sigma dW_{t} \\
...
Itô Integral as: ~\cite{goodman_derivative_securities}
\begin{gather*}
    \int_{0}^{t} f(s) dX_{s}
\end{gather*}
...
equities - we will look at 'AMGN' and 'VOD' for now, but this can be extended to any pair of equities (and even multiple equities through a more advanced strategy). The idea is pretty
...
\end{center}
In the end, we observe that our final portfolio value is around $169.01$, which indicates a $69.01\%$ increase in portfolio value over a time period of $1066$ trading days.
...
\begin{gather*}
    \text{CAGR} = \left ( \frac{V_{\text{final}}}{V_{\text{begin}}} \right )^{\frac{1}{t}} - 1 = \left ( \frac{169.01}{100} \right )^{\frac{1}{4.23}} - 1 = 13.2 \%
\end{gather*}
We can see that our CAGR is around $13.2 \%$, which is relatively high even when compared to some of the best hedge-fund standards. This is even more impressive considering
...
While the baseline returns are impressive, there are some critical limitations of performing statistical arbitrage with our model. Wouldn't it be great if it were this simple to
generate above-market uncorrelated returns, who would need Merton Jump Diffusion, Levy Processes, and the crazy computing hardware they use over at Jane Street to carry out
liquidity provision operations. So let's list some of the limitations this approach entails.
\begin{itemize}
    \item Fixed Averages - The model uses a fixed mean function for computing the Ornstein Uhlenbeck process, it would be far better to use a well-calibrated simple moving average.
    \item Over-reliance on the mean-reverting force - In addition to using the mean reverting force to carry trading signals, it would be better to wait until the time-series reaches
    a supremum and then carry out the trade (this is fairly simple to implement with Gaussian Kernel Convolution).
    \item Ideally, you would also use the mean-reverting force to place weights on different positions in a portfolio optimization algorithm.
    \item The current model has only been backtest on the (AMGM, VOD) pair, ideally you would backtest on all pairs that we computed the cointegration coefficient for.
    \item Use the Hurst Exponent to test long-term mean-reversion.
    \item We omit some factors such as transaction costs.
\end{itemize}

\newpage
\section{Conclusion}

In conclusion, using the Ornstein-Uhlenbeck process to drive signals in a statistical arbitrage strategy with cointegration testing for equity pairs selection can deliver above-market
returns, while being uncorrelated to directional moves on the market. Our strategy, trading the spread between 'NASDAQ: AMGN' and 'NASDAQ: VOD' delivered a CAGR of $13.2 \%$, compared to
the S\&P's $11.24 \%$ return over the 4-year trading period. Please note that past performance does not indicate future results. We have also explored the intuition and
formalization of the Ornstein-Uhlenbeck SDE, and its continuous and discrete time solutions. This work demonstrated the feasability of integrating stochastic process models
...
Code for getting the NASDAQ-100 dataset from the Tiingo API
...
for ticker in symbols:
    ...
    dataframe.to_pickle(f'../data/nasdaq_original/{ticker}.pkl')

    print(dataframe)
\end{lstlisting}

Code for normalizing price data (since this is statarb)
