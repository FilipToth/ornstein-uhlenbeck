...
We will backtest two simple strategies that leverage the Ornstein Uhlenbeck Process. The first one focuses on calculating the mean reverting force $\theta$, which acts as a proxy
for how far the current spread has deviated from the long-term mean, this suggests higher pressure to revert back to its mean. Thus, when $\theta$ is over a predefined treshold, we
...
and short positions. The second strategy is very similar, but instead of using $\theta$, we simply fire our signal when the spread deviates from the mean by a deviation that is greater
(or lesser) than some pre-defined treshold. In both of these strategies, we use a long-short position to isolate the market effect, thus become market neutral, i.e. our position will
...
parameters and the testset will be used to actually perform the backtest and see how our strategy performs.
