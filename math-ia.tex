...
But what is cointegration? Cointegration represents level to which a linear combination
...
spread reverts to the long-term mean (since the mean of the spread is zero), divergence of
...
We assume that our time series follows an Auto-regressive AR(1) model, which is true for equity spreads. AR(1) models are characterized by the following:
\begin{gather*}
    X_{t} = \mu + \phi_{1} X_{t - 1} + \epsilon_{t}
\end{gather*}
Next, we can define our hypotheses, $H_{0} : \phi_{1} = 1$ means that we have a unit root and $H_{1} : \phi_{1} < 1$ means we don't have a unit root.
We can now rewrite the AR(1) equation in terms of the difference between the current and previous value $X_{t} - X_{t - 1} = \Delta X_{t}$. Note that $\delta = \phi_{1} - 1$.
\begin{gather*}
    X_{t} - X_{t - 1} = \mu + X_{t - 1}(\phi_{1} - 1) + \epsilon_{t} \\
    \Delta X_{t} = \mu + \delta X_{t - 1} + \epsilon_{t}
\end{gather*}
We can now restructure our hypotheses in terms of $\delta$ instead of $\phi_{1}$.
\begin{gather*}
    H_{0} : \delta = 0 \\
    H_{1} : \delta < 0
\end{gather*}
Now we can consider that the null hypothesis is true and try to refute it. Since

\subsection{Augmented Dickey-Fuller Test}

We now have a set of residuals $\hat \epsilon_{t}$. This results in another time series comprised of residuals at each time step $t$. We can now use the
Augmented Dickey Fuller Test to check for if the resultant time series is stationary and contains a unit root.
