...
\begin{algorithm}
    ...
    \begin{algorithmic}
        \For{$X_{t}$ \textbf{in} \texttt{testset}}
            \If{$F > \tau_{\text{up}}$}
...
the non-continuous price function. Since out pricing date resembles a function that is discrete and not continuous, we cannot use a limit converging to zero to calculate the derivative
...
approximate the derivative using a technique called Numerical Differentiation.
\begin{gather*}
    f'(x) = \underset{h \to 0}{\lim} \frac{f(x + h) - f(x)}{h} \approx
    \frac{f(x) - f(x - 1)}{\Delta x}
\end{gather*}

% https://notesonai.com/automatic+differentiation
% interesting read https://arxiv.org/abs/2210.08572
