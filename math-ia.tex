...
We consider the the values to be in a pre-computed list $\mathcal{X}$, where $\mathcal{X_{1}}, \mathcal{X_{2}}, \cdots, \mathcal{X_{n}} \in \mathcal{X}$
\begin{gather*}
    MSE = \frac{1}{T} \sum_{i = 1}^{T} \mathcal{X_{i}} = \frac{1}{T} \sum_{i = 1}^{T} X_{i} + -\theta(X_{t} - \mu)e + \mathcal{N}(0, \sigma^{2})
\end{gather*}
